...
//        }
    ...
    /**
     * リストアイテムに対してViewBindingを使わない前時代的バインディングビルダー。
     */
    ...
    /**
     * リストアイテムビューに対してもViewBindingを利用する。
     * リストアイテムビュー内のコントロールに関してタイプセーフなので、今後はこちらを推奨。
     */
...
fun <T> Binder.recyclerViewBinding(
    ...
    view:RecyclerView,
    fn:RecyclerViewBinding.SimpleBuilder<T>.()->Unit) : Binder =
    apply {
...
// T: Item Data Type
// B: ViewBinding Type of List Item

fun <T, B:ViewBinding> Binder.recyclerViewBindingEx(
    owner:LifecycleOwner,
    view:RecyclerView,
    fn:RecyclerViewBinding.ViewBindingBuilder<T,B>.()->Unit) : Binder =
    apply {
